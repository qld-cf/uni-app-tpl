<template>
	<view class="zaiui-home-box" :class="show?'show':''">
		<!--欢迎-->
		<welcome-tip content="欢迎来到PT兰可琪绘本生活馆" :show="true" :c_s="4000" @closeFinish="welcomeClose" />
		...
		<view class="zaiui-view-content" :class="[viewContent.welcome?'welcome':'',headTab.TabCur==0?'show':'']">
			...
			<!--小视频-->
			<!-- <view class="bg-white margin radius padding-sm home-video-height">
				<view class="flex flex-wrap">
					<view class="basis-xs text-center">
						<text class="text-black text-xl text-bold">小视频</text>
					</view>
				</view>
			</view> -->

			<!--广告-->
			<view class="bg-red margin radius padding-sm ad-video-height">
				<view class="flex flex-wrap">
					<view class="basis-xs text-center">
						<text class="text-black text-xl text-bold">广告</text>
					</view>
				</view>
				<!-- <image class="red-envelopes" src="https://ss1.ypshengxian.com/feassets/interview/pics/images/home/sundry/2.png" mode="widthFix" /> -->
			</view>
				...
				<scroll-view scroll-x class="nav z margin-tb-sm">
					<view class="flex text-center">
						<block v-for="(item,index) in goodsTabData.list" :key="index">
							<view class="cu-item flex-sub nf" :class="index==goodsTabData.TabCur?'select':''" @tap="goodsTab"
								:data-id="index">
			...
			<view class="zaiui-tab-list">
				...
				<trends-list :list_data="followTrendsData" :viewBtn="true" @userTap="trends_userTap"
					...
					@appreciateTap="trends_appreciateTap" :show="goodsTabData.TabCur==2" />
		...
		<view class="zaiui-view-content" :class="headTab.TabCur!=0?'show':''">
			...
			<!--宫格分类-->
			<grid-sort-list :list_data="gridSortData" @listTap="gridSortTap" :class="headTab.TabCur==1?'show':'hide'" />
			<grid-sort-list :list_data="gridSortData1" @listTap="gridSortTap" :class="headTab.TabCur==2?'show':'hide'" />
			<grid-sort-list :list_data="gridSortData2" @listTap="gridSortTap" :class="headTab.TabCur==3?'show':'hide'" />
		...
		<view class="zaiui-add-btn-view-box" @tap="rewardTap">
			<button class="cu-btn cuIcon-people bg-red"></button>
		</view>
...
<script>
	...
	export default {
		...
		components: {
			...
			gridSortList, modalImg, trendsList
		},
		data() {
			return {
				...
				gridSortData: [], modalShow: true, gridSortData1: null, gridSortData2: null, followTrendsData: [],
				mountDone: false
			}
		},
		props: {
		...
		mounted() {
			...
			this.gridSortData1 = _home_data.gridSortData1();
			this.gridSortData2 = _home_data.gridSortData2();
			setTimeout(() => {
		...
		methods: {
			...
			setReachBottom() {
				console.log('触底了');
				this.$refs['Message'].info('没有更多咯/(ㄒoㄒ)/~~')
			},
			//欢迎提示关闭事件
			welcomeClose(bol) {
...
<style lang="scss" scoped>
	...
	.zaiui-view-content {
		...
		/* #ifdef H5 */
		margin-top: calc(var(--status-bar-height) + 78rpx);
		/* #endif */
	...
	.zaiui-view-content.welcome {
		...
		/* #ifdef MP */
		margin-top: calc(var(--status-bar-height) + 260rpx);
		/* #endif */
